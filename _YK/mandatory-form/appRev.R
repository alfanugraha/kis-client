...
server <- function(input, output, session) {
  ...
  output$listOfKugi <- renderUI({
    selectInput("kugiName", labelMandatory("Katalog Unsur Geografi Indonesia"), choices=sort(as.character(katalogdata[grep(input$shapeGeom, katalogdata$KUGI),])), selectize=FALSE)
  })
  
  output$listOfTopicCategory <- renderUI({
    selectInput(inputId=as.character(idInfoEntity$vars[13]), label=labelMandatory(as.character(idInfoEntity$name[13])), choices=topiccategory$category, selectize=FALSE, selected=topiccategory$category[1])
  })
  ...
  observe({
    ...
    ###*Verbatim Output for inputfile####
    observeEvent(input$shpData, {
      output$value <- renderText("Shapefile berhasil terupload")
      # output$value <- renderPrint("Shapefile terupload")
    })
  ...
  observeEvent(input$saveButton, {
    ...
    shinyjs::toggleState(id = "saveButton", condition = mandatoryFilled)
